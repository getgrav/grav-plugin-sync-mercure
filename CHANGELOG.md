...
2. [](#bugfix)
    * **Fixed startup crash on Mercure 0.24+** (`invalid transport`). Mercure 0.24 dropped the implicit default transport, so the generated Caddyfile now sets `transport_url bolt://mercure.db` explicitly. The bolt store also restores message replay for clients reconnecting via `Last-Event-ID`.
